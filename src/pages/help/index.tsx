                ...
                <Container className="mb-3 d-flex flex-row flex-wrap">
                  ...
                  <Container className="d-flex flex-column h-100 justify-content-between align-items-start">
                    <div className="mt-3">
                      <Row>
                        Номер карты Сбербанка:{" "}
                        {helpInfo.sber["Номер карты Сбербанка"]}
                      </Row>
                      <Row>
                        прикреплена к номеру:{" "}
                        {helpInfo.sber["прикреплена к номеру"]}
                      </Row>
                      <Row>(оформлена на {helpInfo.sber["оформлена на"]})</Row>
                    </div>

                    <div className="mt-3">
                      {Object.keys(help.else).map((key, index) => (
                        <Row key={`helpInfoRow-${index}`}>
                          {key}:{" "}
                          {(helpInfo.else as Record<string, string>)[key]}
                        </Row>
                      ))}
                    </div>
                  </Container>
